...
impl LcuClient {
    ...
    async fn try_get_current_summoner(&mut self) -> Result<SummonerInfo, String> {
        ...
        Ok(SummonerInfo {
            ...
            display_name: json_value["displayName"].as_str().unwrap_or("Unknown").to_string(),
        ...
        if let Some(queues) = json_value["queues"].as_array() {
            for queue in queues {
                if let Some(queue_type) = queue["queueType"].as_str() {
                    ...
                    if tier != "NONE" && (queue_type == "RANKED_SOLO_5x5" || queue_type == "RANKED_FLEX_SR") {
    ...
    async fn try_get_match_history(&mut self) -> Result<Vec<MatchHistoryGame>, String> {
        ...
        let base_url = format!("{}://127.0.0.1:{}", protocol, port);
        let url = format!("{}/lol-match-history/v1/products/lol/{}/matches?begIndex=0&endIndex=10", base_url, puuid);
                    ...
                    for identity in participant_identities {
                        ...
                        if player_puuid == Some(&puuid) {
                            ...
                            if let Some(stats_array) = participants_stats {
                                if let Some(participant_stats) = stats_array.iter().find(|p| p["participantId"].as_i64().unwrap_or(0) == participant_id) {
                                    let stats = &participant_stats["stats"];
                                    let champion_id = participant_stats["championId"].as_i64().unwrap_or(0) as i32;
